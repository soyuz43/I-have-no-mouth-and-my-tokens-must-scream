...
- [x] ~~Integrate prediction-resolution and expired-duplicate lifecycle semantics into the model-facing scratchpad review prompt.~~ Completed in Slice 7: the prompt documents `PREDICTION_RESOLVE`, its five outcomes, required attributes, content-based identity, and post-expiry resolution timing. Confidence calibration (Slice 8) remains the final open slice.
- [ ] **OPEN (Slice 8, next):** Add evidence-backed prediction evaluation and feed prediction outcomes back into confidence calibration.
...
- [ ] Questions and predictions have complete lifecycles. (Question and prediction resolution operations, archival, archived-ID allocation, expired-duplicate semantics, and model-facing prompt integration are complete. The overall item remains OPEN because Slice 8, evidence-backed prediction evaluation and confidence calibration, is not complete.)
